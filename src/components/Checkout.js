...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      ...
      const response = await axios.post(url, {address: newAddress.value},{
  ...
  const validateRequest = (items, addresses) => {
    if(localStorage.getItem("balance") < getTotalCartValue(items)){
      const warning = "You do not have enough balance in your wallet for this purchase.";
      enqueueSnackbar(warning, {variant: "warning"})
      return false;
    }

    if(!addresses.all.length){
      const warning = "Please add a new address before proceeding.";
      enqueueSnackbar(warning, {variant: "warning"})
      return false;
    }

    if(!addresses.selected.length){
      const warning = "Please select one shipping address to proceed."
      enqueueSnackbar(warning, {variant: "warning"})
      return false
    }
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      const response = await axios.post(url, {addressId: addresses.selected}, {
        headers: {Authorization: `Bearer ${token}`}
      })

      enqueueSnackbar("Order placed succesfully", {variant: "success"})

      const remainingBalance = parseInt(localStorage.getItem("balance")) - getTotalCartValue(items);
      ...
      if(error.response){
        ...
        console.log("final->", error.response.data.message)
      }
      else{
        const error = "Could not place order, check that the backend is running, reachable and returns valid JSON";
