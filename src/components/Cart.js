...
export const generateCartItemsFrom = (cartData, productsData) => {
  // console.log("cartDataInCart->", cartData);
  // console.log("productsDataInCart->", productsData);
  if(!cartData) return;
  const cartItem = cartData.map((item)=>({
    ...item, ...productsData.find((product)=> {
...
export const getTotalCartValue = (items = []) => {
  ...
  const totalCost = items.map((item)=> item.cost * item.qty)

  const sum = totalCost.reduce((initial, current)=>{
